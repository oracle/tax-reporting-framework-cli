...
define(['N/config', 'N/runtime'], function (config, runtime) {
    ...
    SuiteQLBuilder.prototype.build = function (query, context) {
        ...
        if (context.report.subsidiary) {
            subsidiaryFilter = ' AND tl.subsidiary IN ' + "('" + context.report.subsidiary + "')";
        }
        ...
        var isAcctNumberEnabled = config.load('accountingpreferences').getValue('ACCOUNTNUMBERS');
        var localizednumber = isAcctNumberEnabled ? 'acs.acctnumber AS localizednumber, ' : '';
        ...
        var fxamount = isMultiCurrencyEnabled ? 'SUM (tl.foreignamount) AS fxamount, ' : '';
